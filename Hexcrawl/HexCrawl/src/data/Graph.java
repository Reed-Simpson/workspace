...
public class Graph<K> implements Set<K> {
	...
	public Set<K> getAdjacencyList(K key){
		HashSet<K> result = new HashSet<K>();
		Vertex<K> vertex = vertices.get(key);
		if(vertex!=null&&vertex.getAdjacentVertices()!=null&&vertex.getAdjacentVertices().keySet()!=null) {
			for(Vertex<K> v:vertex.getAdjacentVertices().keySet()) {
				result.add(v.key);
			}
		}
		return result;
	...
	public int shortestPath(K start, K finish,LinkedList<K> list){
		PathWrapper<K> result = this.dijkstras(start, finish);
		HashMap<K,K> previous = result.previous;
		if(start.equals(finish)){
			list.add(start);
			//return list;
		}else if(previous.get(finish)==null){
